...
public final class RadiationEvents {
    ...
    private static void applyRadiationExposure(LivingEntity entity) {
        ...
        float maskMultiplier = 1.0F;
        ItemStack gasMask = ItemStack.EMPTY;
        if (entity instanceof Player player) {
            gasMask = player.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.HEAD);
            maskMultiplier = GasMaskItem.protectionMultiplier(gasMask);
            float inventoryRadiation = RadiationExposure.getInventoryRadiationPerSecond(player);
            if (inventoryRadiation > 0.0F) {
                RadiationSystem.contaminate(player, RadiationContaminationType.CREATIVE, inventoryRadiation * maskMultiplier / 20.0F);
            }
        }

        float chunkRadiation = ChunkRadiationManager.getRadiation(entity.level(), entity.blockPosition());
        if (chunkRadiation > 0.0F) {
            RadiationSystem.contaminate(entity, RadiationContaminationType.CREATIVE, chunkRadiation * maskMultiplier / 20.0F);
            if (!gasMask.isEmpty() && entity.tickCount % 20 == 0) {
                GasMaskItem.damageFilter(gasMask, Math.max(1, (int) Math.ceil(chunkRadiation)));
            }
        }
    }
