...
public final class AssemblyMachineMenu extends AbstractContainerMenu {
    ...
    private AssemblyMachineMenu(int containerId, Inventory inventory, AssemblyMachineBlockEntity machine, ContainerData data) {
        ...
        addSlot(new UpgradeSlot(handler, AssemblyMachineBlockEntity.SLOT_UPGRADE_2, 152, 126));
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        } else if (!moveToRecipeGhostSlot(source) && !moveItemStackTo(source, 4, 16, false)) {
            return ItemStack.EMPTY;
        }
        if (source.isEmpty()) {
    ...
    private boolean moveToRecipeGhostSlot(ItemStack source) {
        int selectedRecipe = getSelectedRecipe();
        if (selectedRecipe < 0 || selectedRecipe >= AssemblyMachineRecipes.RECIPES.size()) {
            return false;
        }

        AssemblyMachineRecipes.Recipe recipe = AssemblyMachineRecipes.get(selectedRecipe);
        for (int ingredientIndex = 0; ingredientIndex < recipe.ingredients().size() && ingredientIndex < AssemblyMachineBlockEntity.INPUT_COUNT; ingredientIndex++) {
            ItemStack expected = recipe.ingredients().get(ingredientIndex).stack().get();
            if (!ItemStack.isSameItemSameComponents(source, expected)) {
                continue;
            }

            int targetSlot = AssemblyMachineBlockEntity.INPUT_START + ingredientIndex;
            if (moveItemStackTo(source, targetSlot, targetSlot + 1, false)) {
                return true;
            }
        }
