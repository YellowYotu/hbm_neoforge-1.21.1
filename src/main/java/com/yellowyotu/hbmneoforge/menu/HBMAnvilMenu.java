...
public final class HBMAnvilMenu extends AbstractContainerMenu {
    ...
    public int getAnvilTier() { return anvil.getTier(); }
    ...
    public boolean clickMenuButton(Player player, int id) {
        if (id == HBMAnvilRecipes.size()) {
            ItemStack result = HBMAnvilRecipes.craftFromPlayerInventory(player.getInventory(), selectedRecipe, anvil.getTier());
            if (result.isEmpty()) {
                return false;
            }
    ...
    public boolean stillValid(Player player) {
        return access.evaluate((level, pos) -> level.getBlockState(pos).is(ModBlocks.ANVIL_IRON.get()) || level.getBlockState(pos).is(ModBlocks.ANVIL_STEEL.get()), false);
    }
    ...
    public ItemStack quickMoveStack(Player player, int slotIndex) {
        ...
        } else if (isSteelUpgradeInput(source) || isBrickMaterial(source) || source.is(ModItems.PLATE_IRON.get()) || source.is(net.minecraft.world.item.Items.COPPER_INGOT)) {
